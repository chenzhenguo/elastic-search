...
public class HBaseDataSyncEsObserver extends BaseRegionObserver {
	...
	// LogFactory.getLog(HBaseDataSyncEsObserver.class);
	...
	public void start(CoprocessorEnvironment e) throws IOException {
		...
		// LOG.error("------observer init EsClient ------" +
		// ESClient.getInfo());
		ClusterStateResponse response = ESClient.client.admin().cluster().prepareState().execute().actionGet();
		// 获取所有索引
	...
	public void postPut(ObserverContext<RegionCoprocessorEnvironment> e, Put put, WALEdit edit, Durability durability)
		...
		try {
			...
			// LOG.error("observer put a doc, index [ " + ESClient.indexName +
			// " ]" + "indexId [" + indexId + "] error : "
			// + ex.getMessage());
		}
	}
	...
	public void postDelete(ObserverContext<RegionCoprocessorEnvironment> e, Delete delete, WALEdit edit,
		...
		try {
			...
			// LOG.error(ex);
			// LOG.error("observer delete a doc, index [ " + ESClient.indexName
			// + " ]" + "indexId [" + indexId
			// + "] error : " + ex.getMessage());
			ex.printStackTrace();
	...
	public static void main(String[] args) {
		try {
			Settings settings = Settings.builder().put("cluster.name", ESClient.clusterName)
			...
			json.put("id", "11");
			json.put("title", "es学习");
			json.put("date", "2017-09-04");
			json.put("message", "es学习之路");
			// set hbase family to es
			infoJson.put("info", json);
			ElasticSearchBulkOperator.addUpdateBuilderToBulk(
					client.prepareUpdate("study1", "esstudy", "10").setDocAsUpsert(true).setDoc(infoJson));
