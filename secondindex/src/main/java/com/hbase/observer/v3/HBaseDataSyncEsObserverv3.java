 ...
 * 
 */
public class HBaseDataSyncEsObserverv3 extends BaseRegionObserver {
	private static final Log log = LogFactory.getLog(HBaseDataSyncEsObserverv3.class);
	...
	private static void readConfiguration(CoprocessorEnvironment env) {
		...
		ESClientv3.clusterName = conf.get("es_cluster");
		ESClientv3.nodeHost = conf.get("es_host");
		ESClientv3.nodePort = conf.getInt("es_port", 9300);
...
//		}
	...
	public void start(CoprocessorEnvironment e) throws IOException {
		readConfiguration(e);
		ESClientv3.initEsClient();
		bulkRequestBuilder = ESClientv3.client.prepareBulk();
	}

	public void stop(CoprocessorEnvironment e) throws IOException {
		ESClientv3.closeEsClient();

		// ElasticSearchBulkOperatorv3.shutdownScheduEx();
	}
		...
		try {
			...
			for (Map.Entry<byte[], List<Cell>> entry : familyMap.entrySet()) {
				for (Cell cell : entry.getValue()) {
					...
					Map<String, String> t1 = columnESName.get(key);
					if (t1 != null) {
						// key1是索引名和类型
			...
			for (String key2 : columnESValue.keySet()) {
				...
				bulkRequestBuilder.add(ESClientv3.client.prepareUpdate(key2temp[0], key2temp[1], indexId)
	...
	public void postDelete(ObserverContext<RegionCoprocessorEnvironment> e, Delete delete, WALEdit edit,
		...
		try {
			for (String key : columnESValue.keySet()) {
				...
				DeleteResponse response = ESClientv3.client.prepareDelete(keytemp[0], keytemp[1], indexId).get();
